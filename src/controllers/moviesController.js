...
const API = "https://www.omdbapi.com/?apikey=c1d028be";
        ...
        fetch(`${API}&t=${titulo}&type=movie`)
          ...
          .then(async result => {
             if (result.Response ==="True") {
              const { Title, Released, Genre, Awards, imdbRating, Runtime } =
                result;

              const arrayAwards = Awards ? Awards.match(/\d+/g) : 0;
              const awardsParse = arrayAwards
                ? arrayAwards.map((award) => +award)
                : [];
              let genre_id = null;
              if (Genre.split(",").length) {
                translate(Genre.split(",")[0], { to: "es" }).then(
                  async (response) => {
                    try {
              ...
              try {
                const movie = await db.Movie.create({
                  ...
                  awards: awardsParse.reduce((acum, num) => acum + num, 0),
                  rating: imdbRating || 0,
